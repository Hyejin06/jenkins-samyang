package com.app.samyang.service;

import java.util.List;

import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional; // ✅ 이거 추가

import com.app.samyang.entity.JyCart;
import com.app.samyang.repository.JyCartRepository;

import lombok.RequiredArgsConstructor;

@Service
@RequiredArgsConstructor
public class JyCartService {
    private final JyCartRepository repo;

    public List<JyCart> getCartList(String memberId) {
        return repo.findByMemberId(memberId);
    }

    public void addToCart(JyCart jyCart) {
        repo.save(jyCart);
    }

    public void removeFromCart(Long id) {
        repo.deleteById(id);
    }

    @Transactional // ✅ 트랜잭션 붙여주기!
    public void clearCart(String memberId) {
        repo.deleteByMemberId(memberId);
    }

    public List<JyCart> findByIds(List<Long> selectedItems) {
        return repo.findAllById(selectedItems);
    }

    public JyCart findById(Long id) {
        return repo.findById(id).orElse(null);
    }
}
